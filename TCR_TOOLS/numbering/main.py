...
# -----------------------------
# The code `CDR_FR_RANGES` is a comment in Python. Comments
# are used to provide explanations or annotations in the code
# for better understanding by humans. In this case, the
# comment `CDR_FR_RANGES` is likely indicating a section or
# purpose of the code related to Call Detail Record (CDR)
# ranges for France (FR).
CDR_FR_RANGES = {
    # Alpha-like (A)
    "A_FR1":   (3, 26),
    "A_CDR1":  (27, 38),
...
def fix_duplicate_resseqs_by_insertion_code(pdb_in: str, pdb_out: str) -> None:
    """
    If multiple residues share the same (chain, resseq, iCode) but have different residue
    names, assign distinct iCodes per residue name to disambiguate them.
    - Keeps the first residue name at the original iCode.
    - Additional distinct residue names at that position get iCodes 'A','B','C',...,
      skipping any iCode already in use for that position.
    - Does NOT change anything if residue names are identical (e.g., altLocs).
    """
    def is_atom(line: str) -> bool:
        return line.startswith("ATOM  ") or line.startswith("HETATM")

    def fields(line: str):
        # PDB fixed columns (0-based, end-exclusive)
        # altLoc: [16], resName: [17:20], chain: [21], resSeq: [22:26], iCode: [26]
        resname = line[17:20]
        chain   = line[21]
        resseq  = line[22:26]   # keep as 4-char string
        icode   = line[26]
    ...
    def icodes_iter():
        ...
        for c in letters:
            yield c
        # then double (rare, but safe)
        for c1 in letters:
            for c2 in letters:
                yield c1 + c2

    # 1) First pass: collect distinct residue names per (chain, resseq, icode)
    from collections import OrderedDict, defaultdict
    lines = open(pdb_in, "r").readlines()

    resnames_per_key = defaultdict(OrderedDict)  # key -> ordered set (via OrderedDict keys)
    for ln in lines:
        if not is_atom(ln):
            continue
        resname, chain, resseq, icode = fields(ln)
        key = (chain, resseq, icode)
        if resname not in resnames_per_key[key]:
            resnames_per_key[key][resname] = True

    # 2) Build per-key mapping resname -> new icode only for conflict keys (>1 resnames)
    mapping = {}  # key -> dict(resname -> new_icode)
    for key, od in resnames_per_key.items():
        distinct_resnames = list(od.keys())
        if len(distinct_resnames) <= 1:
            continue  # no conflict: same residue name only
        chain, resseq, orig_icode = key
        used = set()
        resname_to_icode = {}

        # Keep the first residue name on the original iCode
        first_res = distinct_resnames[0]
        resname_to_icode[first_res] = orig_icode
        used.add(orig_icode)

        # Assign new, unused codes to additional residue names
        gen = icodes_iter()
        for res in distinct_resnames[1:]:
            code = orig_icode  # start from original, then bump
            # choose next available not equal to orig or already used
            while True:
                code = next(gen)
                if code not in used:
                    break
            resname_to_icode[res] = code
            used.add(code)

        mapping[key] = resname_to_icode

    # 3) Second pass: rewrite only conflicting keys
    out = []
    for ln in lines:
        if not is_atom(ln):
            out.append(ln)
        ...
        key = (chain, resseq, icode)
        if key in mapping:
            new_icode = mapping[key].get(resname, icode)
            # rewrite column 27 (index 26) with the first char of the new code
            # (PDB iCode is a single character; if we ever produced multi-char, keep the first)
            ln = ln[:26] + (new_icode[0] if new_icode else " ") + ln[27:]
        out.append(ln)
