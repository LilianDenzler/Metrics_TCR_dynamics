...
then plot per-state distributions and scatter plots colored by V/J alpha/beta gene combinations.

Coloring key:
  germline_vj_pair = (alphaV-alphaJ) | (betaV-betaJ)

Outputs (per state):
  - V/J combo count bar chart (top N + other)
  - For each metric: overlay histogram + boxplot (top N + other)
  - Scatter plots (selected pairs) colored by germline_vj_pair
      * Legend labels include counts per germline (e.g., "TRAV..-TRAJ..|TRBV..-TRBJ.. (n=12)")

Also writes:
  - CSV of all computed metrics with parsed germlines
  - summary_counts_by_state.csv
"""
...
TOP_N_COMBOS = 50          # keep plots readable; others collapsed to "other"
...
# --------------------------------------------------------------------
# 3) SCATTER PLOTS (COLORED BY GERMLINE, LEGEND SHOWS COUNTS)
# --------------------------------------------------------------------
def _default_scatter_pairs(metrics_available: List[str]) -> List[Tuple[str, str]]:
    """
    Choose a small, interpretable set of scatter pairs.
    Only returns pairs where both columns exist.
    """
    candidates = [
        ("BA", "dc"),
        ("BC1", "BC2"),
        ...
        ("BA", "AC1"),
        ("BC1", "AC1"),
        # If bend metrics exist, these are typically the most informative:
        ("alpha_cdr3_bend_abs_deg", "alpha_cdr3_apex_height_A"),
        ("beta_cdr3_bend_abs_deg", "beta_cdr3_apex_height_A"),
        ("alpha_cdr3_bend_signed_deg", "alpha_cdr3_apex_height_A"),
        ("beta_cdr3_bend_signed_deg", "beta_cdr3_apex_height_A"),
    ]
    out = []
    s = set(metrics_available)
    for x, y in candidates:
        if x in s and y in s:
            out.append((x, y))
    return out

def plot_scatter_by_genecombo(
    df: pd.DataFrame,
    state: str,
    out_dir: Path,
    ...
    """
    Scatter plots colored by germline combo.
    Legend labels include counts per germline *for the plotted (x,y finite) points*.
    """
    ensure_dir(out_dir)

    # Collapse categories overall for this state (keeps colors consistent across plots)
    combos_collapsed, cat_order = _collapse_categories(df[combo_col], top_n=top_n, min_count=min_count)
    color_map = _get_color_map(cat_order)
    ...
    for xcol, ycol in scatter_pairs:
        ...
        dd = d.loc[mask, ["_combo_plot"]].copy()
        dd["x"] = x.loc[mask].values
        dd["y"] = y.loc[mask].values

        # counts per category for THIS scatter
        counts = dd["_combo_plot"].value_counts()
        cats_present = [c for c in cat_order if counts.get(c, 0) > 0]

        fig = plt.figure(figsize=(9.2, 6.8))
        ax = fig.add_subplot(1, 1, 1)

        for cat in cats_present:
            sub = dd[dd["_combo_plot"] == cat]
            ax.scatter(
                sub["x"].values,
                sub["y"].values,
                s=size,
                alpha=alpha,
                c=[color_map.get(cat, (0.6, 0.6, 0.6, 0.6))],
                edgecolors="none",
                label=f"{cat} (n={int(counts.get(cat, 0))})",
            )

        ax.set_title(f"{state}: {ycol} vs {xcol} (colored by V/J combo)")
        ax.set_xlabel(xcol)
        ax.set_ylabel(ycol)
        ax.legend(loc="best", fontsize=8, frameon=True)

        fig.tight_layout()
        out_path = out_dir / f"{state}__scatter__{ycol}_vs_{xcol}__by_germline_vj_pair.png"
...
def main():
    ...
    for state in ["unbound", "bound"]:
        ...
        # 2) distributions per metric
        plot_distributions_by_genecombo(
            df=df_state,
            state=state,
            out_dir=state_dir / "distributions",
        ...
        # 3) scatter plots
        scatter_pairs = _default_scatter_pairs(metrics_available=metrics_to_plot)
        plot_scatter_by_genecombo(
            df=df_state,
            state=state,
            out_dir=state_dir / "scatter",
...
if __name__ == "__main__":
    ...
    OUTPUT_DIR  = Path("/workspaces/Graphormer/TCR_Metrics/pipelines/analyse_geometry/TCR3d_analysis_angles/scatter_plots")
    main()
